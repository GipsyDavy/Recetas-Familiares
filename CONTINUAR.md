...
- Prueba de envio real disparada: `POST /auth/password-reset/request` para la cuenta real del usuario -> 202, sin errores de mail en journalctl. PENDIENTE: confirmacion del usuario de que el correo llego (mirar spam la primera vez). El token de ese correo caduca en 30 min y NO cambia la password si no se confirma.
- Cierre E2E de CRIT-2 (3 flujos con email real) queda a un paso: confirmar recepcion y, si se quiere, probar reset completo y verificacion de email desde la app.
